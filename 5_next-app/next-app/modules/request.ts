...
const AxiosClient = (options?: AxiosRequestConfig) => {
	...
	return {
	  client,
	  get: <T = any, R = AxiosResponse<T>>(url: string, params?: Record<string, unknown>): Promise<R> => {
		return client.get(url, params)
	  },
	  post: <T = any, R = AxiosResponse<T>>(url: string, data?: Record<string, unknown>): Promise<R> => {
